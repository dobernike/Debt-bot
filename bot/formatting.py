...
def format_help() -> str:
    return (
        ...
        "<code>/history</code> — последние 10 транзакций\n\n"
        "<b>Настройки:</b>\n"
        "<code>/currency</code> — показать дефолтную валюту\n"
        "<code>/currency RUB</code> — установить рубль как дефолт\n"
        "<code>/currency USD</code> — установить доллар как дефолт\n\n"
        "<i>Валюта по умолчанию — USD (если не изменена).</i>"
    )
